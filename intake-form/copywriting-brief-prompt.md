...
2. **Website homepage hero copy**: a headline and subhead built from their
   Core People answers, speaking directly to their Point A (the struggle)
   and pointing at Point B (what they want to feel instead), positioning
   the client's offer as what moves someone from one to the other. Use
   their "One-Liner" answer as a starting point for the headline if it's
   usable as-is.
...
4. **5 blog post title + one-paragraph outline ideas**: built from the real
   questions they said people ask them over and over, plus the content
...
- The intake form's `core_story`, `core_people`, and `connect` sections are
  built from a three-chapter storytelling framework (Tipping Point, then
  Journey, then Purpose; Knowing, then Point A/Point B, then Collecting;
  Pathway, then Presence, then Process), so the copy asks above map
  directly onto that structure instead of a generic brief.
...
- `resources/storytelling-content-templates.csv` is a bonus pack of 100+
  fill-in-the-blank content hooks, tagged Educational, Inspirational, or
  Entertaining. It's a shortcut for step 3 above, not a replacement for it,
  the blanks still get filled with real, specific details from Ali's
  answers, never generic placeholders.

## Making this repeatable for future clients

1. Duplicate the `intake-form/` folder for the new client.
2. Edit `questions.js`, swap in whatever framework you're using for that
   client (or keep this one).
3. Update the theme colors/fonts/images at the top of `style.css`.
